...
def _text_to_sequence(text, cleaner_names, as_token):
    ...
    while len(text):
        ...
        if not m:
            text, _text = _clean_text(text, cleaner_names)
            sequence += _symbols_to_sequence(text)
            break
        sequence += _symbols_to_sequence(_clean_text(m.group(1), cleaner_names)[0])
    ...
    if as_token:
        return sequence_to_text(sequence, combine_jamo=True), _text
    else:
        return np.array(sequence, dtype=np.int32), _text
...
def _clean_text(text, cleaner_names):
    ...
    for name in cleaner_names:
        ...
        text, _text = cleaner(text) # '존경하는' --> ['ᄌ', 'ᅩ', 'ᆫ', 'ᄀ', 'ᅧ', 'ᆼ', 'ᄒ', 'ᅡ', 'ᄂ', 'ᅳ', 'ᆫ', '~']
    return text, _text
